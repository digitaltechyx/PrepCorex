...
export async function appendOutboundTrackerLabelPhoto(input: {
  id: string;
  photo: TrackerLabelPhoto;
}): Promise<OutboundTrackerEntry | null> {
  const db = getAdminDb();
  const FieldValue = getAdminFieldValue();
  const ref = db.collection(OUTBOUND_TRACKING_COLLECTION).doc(input.id);
