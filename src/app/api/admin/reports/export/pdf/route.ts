...
import { buildAdminReportComparisonPdf, buildAdminReportPdf } from "@/lib/admin-reports-pdf";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const scopeSlug = (name: string) => name.replace(/\s+/g, "-");
    ...
    if (reportType === "comparison") {
      ...
      if (!from || !to || !compareFrom || !compareTo || allTime) {
        return NextResponse.json(
          { error: "Comparison PDF requires from/to and compareFrom/compareTo date ranges." },
          { status: 400 }
        );
      }
      ...
      const pdfBytes = buildAdminReportComparisonPdf(comparison);
      const scope = comparison.scope.allClients
        ? "all-clients"
        : scopeSlug(comparison.scope.clientName || "client");
      const filename = `prepcorex-report_comparison_${scope}_${format(from, "yyyy-MM-dd")}_vs_${format(new Date(compareFrom), "yyyy-MM-dd")}.pdf`;
    ...
    const pdfBytes = buildAdminReportPdf(summary, reportType);
    const scope = summary.scope.allClients ? "all-clients" : scopeSlug(summary.scope.clientName || "client");
    const filename = allTime
      ? `prepcorex-report_${reportType}_${scope}_all-time.pdf`
      : `prepcorex-report_${reportType}_${scope}_${format(from, "yyyy-MM-dd")}.pdf`;
