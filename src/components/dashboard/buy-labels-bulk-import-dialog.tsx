...
export function BuyLabelsBulkImportDialog({
  ...
  const handleAddToCart = async () => {
    ...
    const cartItems: BuyLabelCartImportItem[] = [];
    const rateErrors: BuyLabelsBulkRowError[] = [];
    ...
    try {
      for (let i = 0; i < validRows.length; i++) {
        ...
        if (!response.ok) {
          ...
          rateErrors.push({ rowNumber: row.rowNumber, message });
          continue;
        }
        ...
        cartItems.push({
          ...
          parcel: row.parcel,
          selectedRate,
          shipmentId: data.shipment_id || selectedRate.shipment || null,
        });
      }
      ...
      if (cartItems.length === 0) {
        ...
        setRowErrors((prev) => [...prev, ...rateErrors]);
        return;
      }
      ...
      if (rateErrors.length > 0) {
        toast({
          ...
          description: `${cartItems.length} label${cartItems.length === 1 ? "" : "s"} added to cart. ${rateErrors.length} row${rateErrors.length === 1 ? "" : "s"} failed.`,
        });
        setRowErrors((prev) => [...prev, ...rateErrors]);
      } else {
        toast({
              ...
              {needsLocationPicker && (
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Choose which warehouse From address to pre-fill in the template.
                  </p>
                </div>
              )}
