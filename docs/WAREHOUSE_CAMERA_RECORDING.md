...
- While live, the operator can end hands-free by holding an **open palm for two seconds** (on-device MediaPipe gesture recognition). On supported browsers, the operator can say **"End session"** or **"Stop recording"**. Voice listening starts immediately, uses interim mobile speech results, accepts common speech transcriptions, and shows the latest heard phrase for field diagnosis.
